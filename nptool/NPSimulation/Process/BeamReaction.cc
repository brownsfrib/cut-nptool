...
G4bool NPS::BeamReaction::ModelTrigger(const G4FastTrack& fastTrack) {
  ...
  G4ThreeVector P = fastTrack.GetPrimaryTrack()->GetPosition();
  ...
  double to_exit = solid->DistanceToOut(P_loc, V_loc);
  double to_entrance = solid->DistanceToOut(P_loc, -V_loc);
  ...
  if (is_first) {
    ...
    // random Z along the track in the reaction volume
...
void NPS::BeamReaction::DoIt(const G4FastTrack& fastTrack, G4FastStep& fastStep) {
  ...
  // Positions of the last step before the reaction
  ...
  // Takes into account the m_length distance traveled within the last step
  double reac_energy = SlowDownBeam(PrimaryTrack->GetParticleDefinition(), energy, m_length, material);
  localPosition = localPosition + localdir * m_length;
  worldPosition = worldPosition + pdirection * m_length;
  ...
  ////////////////////////////////////////
    ...
    fastStep.CreateSecondaryTrack(particleLight, worldPosition, time, false);

    G4DynamicParticle particleTarget(LightName, momentum_kineTarget_world, EnergyTarget);
    fastStep.CreateSecondaryTrack(particleTarget, worldPosition, time, false);
  ...
  // QFS
    ...
    if (m_QFS.GetShoot1()) {
      ...
      fastStep.CreateSecondaryTrack(particle1, worldPosition, time, false);
    }

    if (m_QFS.GetShoot2()) {
      G4DynamicParticle particle2(Light2Name, momentum_kine2_world, TKE2);
      fastStep.CreateSecondaryTrack(particle2, worldPosition, time, false);
    }
    if (m_QFS.GetShootB()) {
      if (m_QFS.GetDeexcitation()) {
        ...
        for (unsigned int i = 0; i < sizeFrag; i++) {
          ...
          fastStep.CreateSecondaryTrack(particleFrag, worldPosition, time, false);
        }
      }
      else {
        G4DynamicParticle particleB(HeavyName, momentum_kineB_world, TKEB);
        fastStep.CreateSecondaryTrack(particleB, worldPosition, time, false);
      }
    }
  ...
  //////////////////////////
    ...
    if (m_PhaseSpace.SetBeamLV(pdirection.x(), pdirection.y(), pdirection.z(), energy)) {
      ...
      for (unsigned int i = 0; i < size; i++) {
        ...
        fastStep.CreateSecondaryTrack(particle, worldPosition, time, false);
      }
    }
  }
