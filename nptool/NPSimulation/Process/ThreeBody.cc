...
#include <string>
////////////////////////////////////////////////////////////////////////////////
NPS::ThreeBody::ThreeBody(G4String modelName, G4Region* envelope) : G4VFastSimulationModel(modelName, envelope) {
  ReadConfiguration();
  m_PreviousEnergy = 0;
  m_PreviousLength = 0;
}
...
////////////////////////////////////////////////////////////////////////////////
NPS::ThreeBody::ThreeBody(G4String modelName) : G4VFastSimulationModel(modelName) {}
  ...
  if (blocks.size() > 0) {
    ...
    for (unsigned int i = 0; i < blocks.size(); i++) {
      ...
      if (blocks[i]->HasToken("CompoundNucleusEnergy"))
        m_CompoundNucleusEnergy = blocks[i]->GetDouble("CompoundNucleusEnergy", "MeV");

      if (blocks[i]->HasToken("CompoundNucleusStateWidth"))
        m_CompoundNucleusStateWidth = blocks[i]->GetDouble("CompoundNucleusStateWidth", "MeV");
      ...
      if (blocks[i]->HasToken("IntermediaryStateWidth"))
        m_IntermediaryStateWidth = blocks[i]->GetDouble("IntermediaryStateWidth", "MeV");
      ...
      if (blocks[i]->HasToken("ExcitationEnergyHeavy"))
        m_ExcitationEnergyHeavy = blocks[i]->GetDouble("ExcitationEnergyHeavy", "MeV");
      ...
      if (blocks[i]->HasToken("UserEnergyCS")) {
        ...
        if (m_UserEnergyCS) {
          ...
          m_EnergyCS = (TF1*)m_FileEnergyCS->FindObjectAny(m_EnergyCSName.c_str());
    ...
    m_end = false;
    m_previousE = -1000;
    fIntermediaryStateEnergy = GetIntermediaryStateEnergy(m_IntermediaryState, m_IntermediaryStateWidth);
  }
  else
    m_active = false;
}
...
G4bool NPS::ThreeBody::IsApplicable(const G4ParticleDefinition& particleType) {
  ...
  if (!m_active)
    ...
    if (particleName.find(m_BeamName) != std::string::npos && blocks.size() > 0) {
      return true;
    }
  }
  return false;
...
G4bool NPS::ThreeBody::ModelTrigger(const G4FastTrack& fastTrack) {
  ...
  G4ThreeVector P = PrimaryTrack->GetPosition();
  G4ThreeVector V_loc = fastTrack.GetPrimaryTrackLocalMomentum().unit();
  G4ThreeVector P_loc = fastTrack.GetPrimaryTrackLocalPosition();
  G4VSolid* solid = fastTrack.GetPrimaryTrack()->GetVolume()->GetLogicalVolume()->GetSolid();
  double in = solid->DistanceToOut(P_loc, V_loc);
  double out = solid->DistanceToOut(P_loc, -V_loc);
...
void NPS::ThreeBody::DoIt(const G4FastTrack& fastTrack, G4FastStep& fastStep) {
  ...
  worldPosition = worldPosition - pdirection * length;
  localPosition = localPosition - localdir * length;
  ...
  fastStep.SetPrimaryTrackFinalKineticEnergyAndDirection(0, pdirection, false);
  fastStep.SetPrimaryTrackFinalPosition(worldPosition, false);
  fastStep.SetTotalEnergyDeposited(0);
  fastStep.SetPrimaryTrackFinalTime(time);
  fastStep.KillPrimaryTrack();
  fastStep.SetPrimaryTrackPathLength(0.0);

  double Ecm = sqrt(m_N1.Mass() * m_N1.Mass() + m_N2.Mass() * m_N2.Mass() + 2 * m_N2.Mass() * (energy + m_N1.Mass())) -
               m_N1.Mass() - m_N2.Mass();
  if (m_UserEnergyCS == 0) {
  ...
  if (Allowed) {
    ...
    if (m_UserEnergyCS == 0) {
      T1 = (pow((Ecm + m_N1.Mass() + m_N2.Mass()), 2.) - (m_N1.Mass() * m_N1.Mass() + m_N2.Mass() * m_N2.Mass())) /
               (2. * m_N2.Mass()) -
           m_N1.Mass();
    }
    ...
    if (m_type == "sequential") {
      ...
      G4ParticleDefinition* Light1 =
          G4ParticleTable::GetParticleTable()->GetIonTable()->GetIon(m_N3.GetZ(), m_N3.GetA(), 0);

      G4ParticleDefinition* Light2 =
          G4ParticleTable::GetParticleTable()->GetIonTable()->GetIon(m_N5.GetZ(), m_N5.GetA(), 0);

      G4ParticleDefinition* Heavy =
          G4ParticleTable::GetParticleTable()->GetIonTable()->GetIon(m_N6.GetZ(), m_N6.GetA(), m_ExcitationEnergyHeavy);
      ...
      if (m_ShootLight1) {
        ...
        fastStep.CreateSecondaryTrack(particleLight1, worldPosition, time, false);
      }

      if (m_ShootLight2) {
        G4DynamicParticle particleLight2(Light2, p5G4.unit(), T5);
        fastStep.CreateSecondaryTrack(particleLight2, worldPosition, time, false);
      }
      if (m_ShootHeavy) {
        G4DynamicParticle particleHeavy(Heavy, p6G4.unit(), T6);
        fastStep.CreateSecondaryTrack(particleHeavy, worldPosition, time, false);
      }
      ///////////////////////////////////////
...
void NPS::ThreeBody::GetDecay(TVector3 p1, double Etot1, unsigned int nDecay, double* masses, double* T, double* theta,
                              double* phi) {
  TGenPhaseSpace PhaseSpace;
  TLorentzVector NucleiToDecay(p1.x() / 1000., p1.y() / 1000., p1.z() / 1000., Etot1 / 1000.);
...
TF1* NPS::ThreeBody::GetIntermediaryStateEnergy(double Er, double Width) {
  ...
  TF1* f = new TF1("f", "1. / (pow((x - [0]), 2.) + pow(([1] / 2.), 2.))", 0, 100);
  f->SetParameter(0, Er);
