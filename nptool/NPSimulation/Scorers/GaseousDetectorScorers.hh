...
namespace GaseousDetectorScorers {
  ...
  // --- ------------------------------------------- --- //
      public:
        GaseousDetectorData(const double& Energy,
			    ...
			    const vector<double>& posY,
                            const vector<double>& posZ, 
                            const vector<double>& DE,
			    const vector<double>& Ekin,
	...
	vector<double> m_StepPosX;
	vector<double> m_StepPosY;
        vector<double> m_StepPosZ;
        vector<double> m_EnergyLossPerStep;
	vector<double> m_EkinPerStep;
	...
	inline vector<double> GetStepPosX() const {return m_StepPosX;}
	inline vector<double> GetStepPosY() const {return m_StepPosY;}
	...
	inline vector<double> GetEkinPerStep() const {return m_EkinPerStep;}
	...
	inline void SetStepPosX(const double& x){m_StepPosX.push_back(x);}
	inline void SetStepPosY(const double& y){m_StepPosY.push_back(y);}
	...
	inline void SetEkinPerStep(const double& E){m_EkinPerStep.push_back(E);}
        inline void SetStepTime(const double& t){m_StepTime.push_back(t);}
  };
  ...
  // --- ----------------------- --- //
    ...
    public:
      ...
      inline void Set(const double& Energy, 
        ...
        m_Data.push_back(GaseousDetectorData(Energy,Time,Xpos,Ypos,Zpos,Nesting,name,tMass,tCharge,tID,x,y,z,DE,Ekin,t));
      };
  ...
  class PS_GaseousDetector : public G4VPrimitiveScorer{
	...
	vector<double> t_StepPosX;
	vector<double> t_StepPosY;
        vector<double> t_StepPosZ;
        vector<double> t_EnergyLossPerStep;
	vector<double> t_EkinPerStep;
    ...
    public:
      ...
      inline vector<double> GetStepPosX(const unsigned int& i) const {return m_Data[i]->GetStepPosX();}
      inline vector<double> GetStepPosY(const unsigned int& i) const {return m_Data[i]->GetStepPosY();}
      ...
      inline vector<double> GetEkinPerStep(const unsigned int& i) const {return m_Data[i]->GetEkinPerStep();}
