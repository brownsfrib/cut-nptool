...
namespace PlasticBar {
  ...
  class PlasticBarData {
   public:  
    PlasticBarData(const double& Energy, const double& Light, const double& PositionY, const double& Time,
                   const std::vector<unsigned int>& Nesting, const double& PositionX, const int& ParticleID, const int& ParentID)
    {
  ...
  class PlasticBarDataVector {
   ...
   public:
    ...
    inline void Set(const double& Energy, const double& Light, const double& PositionY, const double& Time,
                    const std::vector<unsigned int>& Nesting, const double& PositionX, const int& ParticleID, const int& ParentID) {
      m_Data.push_back(PlasticBarData(Energy, Light, PositionY, Time, Nesting, PositionX, ParticleID, ParentID));
    };
  ...
  class PS_PlasticBar : public G4VPrimitiveScorer {
   ...
   public:
    ...
    inline int GetParticleID(const unsigned int& i) {return m_Data[i]->GetParticleID();};
    inline int GetParentID(const unsigned int& i) {return m_Data[i]->GetParentID();};
