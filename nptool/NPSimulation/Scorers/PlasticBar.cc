...
G4bool PS_PlasticBar::ProcessHits(G4Step* aStep, G4TouchableHistory*) {
  ...
  if (processname != "Transportation") {
    ...
    //cout << aStep->GetTrack()->GetTrackID() << "     " << aStep->GetTrack()->GetParentID() << "   " << t_Energy << "    " << particlename << endl;
    ...
    if (particlename != "neutron" && Z < 7) {
      ...
      t_ParticleID = aStep->GetTrack()->GetTrackID();
      t_ParentID = aStep->GetTrack()->GetParentID();
      t_Level.clear();
      ...
      if (t_Time < 150) {
        ...
        if (it == m_Data.end()) {
          m_Data.Set(0.0, 0.0, t_PositionY, t_Time, t_Level, t_PositionX, t_ParticleID, t_ParentID);
