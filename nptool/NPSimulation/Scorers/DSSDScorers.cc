...
G4bool PS_Rectangle::ProcessHits(G4Step* aStep, G4TouchableHistory*) {
  ...
  if (m_ScorerMethod == "new") {
    return ProcessHitsNew(aStep);
  }
  else if (m_ScorerMethod == "old") {
    t_Energy = aStep->GetTotalEnergyDeposit();
    t_Time = aStep->GetPreStepPoint()->GetGlobalTime();
    ...
    cerr << "ERROR in PS_Rectangle: Scorer method unknown" << endl;
    return FALSE;
  }
}
...
G4bool PS_Annular::ProcessHits(G4Step* aStep, G4TouchableHistory*) {
  ...
  // vector<DSSDData>::iterator it;
  //  Ring
  auto it = m_HitRing.find(DSSDData::CalculateIndex(t_StripRingNumber, t_DetectorNumber));
  if (it != m_HitRing.end()) {
    it->Add(t_Energy);
  }
  else
