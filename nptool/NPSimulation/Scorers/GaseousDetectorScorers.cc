...
G4bool PS_GaseousDetector::ProcessHits(G4Step* aStep, G4TouchableHistory*) {
  ...
  double step_posX = aStep->GetPostStepPoint()->GetPosition().x(); // x coordinate at the end of the step.
  double step_posY = aStep->GetPostStepPoint()->GetPosition().y(); // y coordinate at the end of the step.
  double step_posZ = aStep->GetPostStepPoint()->GetPosition().z(); // z coordinate at the end of the step.
  //t_Ekin = aStep->GetKineticEnergy();
  t_Ekin = aStep->GetPostStepPoint()->GetKineticEnergy();
  ...
  t_StepPosX.push_back(step_posX);
  t_StepPosY.push_back(step_posY);
  t_StepPosZ.push_back(step_posZ);
  t_EnergyLossPerStep.push_back(t_Energy);
  t_EkinPerStep.push_back(t_Ekin);
  ...
  if (it != m_Data.end()) {
    ...
    it->SetStepPosX(step_posX);
    it->SetStepPosY(step_posY);
    it->SetStepPosZ(step_posZ);
    it->SetEnergyLossPerStep(t_Energy);
    it->SetEkinPerStep(t_Ekin);
    it->SetStepTime(t_Time);
  } 
  else {
    // first step: initialize a new m_Data vector<GaseousDetectorData>
    // t_Time [ns] is the time at first step
    m_Data.Set(t_Energy, t_Time, x, y, z, t_Level, t_ParticleName, t_ParticleA, t_ParticleZ, t_TrackID, t_StepPosX, t_StepPosY, t_StepPosZ, t_EnergyLossPerStep, t_EkinPerStep, t_StepTime);
  }
