...
G4Material* MaterialManager::GetMaterialFromLibrary(string Name, double density) {
  ...
  if (it == m_Material.end()) {
    ...
    // Heavy water
    else if (Name == "D2O" || Name == "HeavyWater") {
      if (!density)
        density = 1.11 * g / cm3;
      ...
      if (!density)
        density = 0.964 * g / cm3;
      G4Material* material = new G4Material("NPS_" + Name, density, 2);
      material->AddElement(GetElementFromLibrary("C"), 435); //4.35e22 atoms per cm3
      material->AddElement(GetElementFromLibrary("H"), 543); //5.43e22 atoms per cm3
      m_Material[Name] = material;
      return material;
    }
      ...
      if (!density)
        density = 15.1 * g / cm3;
      G4Element* Cf252 = new G4Element("Cf252", "Cf252", 1);

      G4Isotope* isotope = new G4Isotope("252Cf", 98, 252);
      Cf252->AddIsotope(isotope, 1);

      G4Material* material = new G4Material("NPS_" + Name, density, 1, kStateSolid, 293.15 * kelvin);
      material->AddElement(Cf252, 1);
      m_Material[Name] = material;
      return material;
    }
    ...
    else if (Name == "He3_gas") {
      if (!density)
        density = 0.0001665 * g / cm3; // room temp, 1 atm but it is wrong. you should always specify a density
      G4Material* material = new G4Material("NPS_" + Name, density, 1);
      material->AddElement(GetElementFromLibrary("He3"), 1); // only one atom of 3He
      m_Material[Name] = material;
      return material;
    }
    else if (Name == "O2_gas") {
    ...
    else if (Name == "concrete") { // Formula from Lise++
      if (!density)
        density = 2.34 * g / cm3;
      G4Material* material = new G4Material("NPS_" + Name, density, 5);
      material->AddElement(GetElementFromLibrary("C"), 23./97);
      material->AddElement(GetElementFromLibrary("O"), 40./97);
      material->AddElement(GetElementFromLibrary("Si"), 12./97);
      material->AddElement(GetElementFromLibrary("Ca"), 12./97);
      material->AddElement(GetElementFromLibrary("H"), 10./97);
      m_Material[Name] = material;
      return material;
    }
...
G4Element* MaterialManager::GetElementFromLibrary(string Name) {
  if (Name == "D" || Name == "d") {
    ...
    if (!m_He3) {
      ...
      G4Isotope* isotope = new G4Isotope(Name.c_str(), 2, 3, 3.0160293 * g / mole);
    ...
    if (!m_Li6) {
      ...
      G4Isotope* isotope = new G4Isotope(Name.c_str(), 3, 6, 6.01512289 * g / mole);
    ...
    if (!m_Li7) {
      ...
      G4Isotope* isotope = new G4Isotope(Name.c_str(), 3, 7, 7.01600343 * g / mole);
