...
// Sam's addition of Viola kinematics
...
void PrimaryGeneratorAction::ReadEventGeneratorFile(string Path){
    ...
    // Sam's addition of Viola kinematics - 7/22/25
    blocks.clear();
    blocks = parser.GetAllBlocksWithToken("ViolaFission"); 
    if (blocks.size()>0) { 
        NPS::VEventGenerator* myEventGenerator = new EventGeneratorViola(); 
        myEventGenerator->ReadConfiguration(parser); 
        myEventGenerator->InitializeRootOutput(); 
        m_EventGenerator.push_back(myEventGenerator); 
    } 
