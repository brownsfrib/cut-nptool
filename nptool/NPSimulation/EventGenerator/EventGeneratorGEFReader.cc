  ...
  for(auto& par : m_Parameters) {
	...
	// === Extract data of the current line
	  ...
	  // Compound nucleus (after pre-fission emission)
          ...
          // antia Read neutrons energy to correct Ex  
          if(DataLine.size()>data_length)
          {
            string ParticleList1 = to_string(DataLine.at(data_length));
            int idx1=0;
            for(char type: ParticleList1){
              if(type=='1' || type=='3' || type=='5'){
                double Tn = DataLine.at(data_length + 1 + idx1*3);
                idx1++;

		//cout << A_CN << " " << idx1 << endl;
		NPL::Particle nucleusParent(Z_CN, A_CN+idx1);
		double massParent = nucleusParent.Mass();
		NPL::Particle nucleusSon(Z_CN, A_CN);
		double massSon = nucleusSon.Mass();
		NPL::Nucleus neutron("1n");
                double massN = neutron.Mass();
		//double Sn = massSon + (idx1)*massN - massParent;

		double Sn = nucleusParent.GetSXn(1);
		//Ex += Sn + Tn;
		Ex += Sn + Tn + sqrt(Tn*Tn + 2*Tn*massN + massSon*massSon) - massSon;
		//Ex += Sn + Tn + Tn*massN/massSon;
		//cout << "Ex corr = " << Ex << endl;
	  ...
	  if(DataLine.size()>data_length)
        ...
        // NEUTRONS: first line has 0 at DataLine.(0) and gives the list of the post-scission neutrons  
          ...
          {// Prompt fission neutron treatment
    ...
    //for a uniform disk of radius R
    //TRandom3 randGen;
    //double r = par.m_R * sqrt(randGen.Uniform()); // Random radius (scaled to maintain uniform distribution in area)
    //double theta = randGen.Uniform(0, 2 * M_PI); // Random angle
    //x0 = par.m_x0 + r * cos(theta);
    //y0 = par.m_y0 + r * sin(theta);
